...
# Новые функции для работы с фото
async def analyze_photo_with_gpt(image_bytes: bytes, text_prompt: str = "") -> str:
    """Анализирует фото с помощью GPT-4 Vision"""
    try:
        base64_image = base64.b64encode(image_bytes).decode('utf-8')
        
        system_prompt = """Ты опытный ветеринар и эксперт по уходу за домашними животными. 
        При анализе фотографий питомцев:
        1. Детально описывай что видишь (породу, состояние шерсти, кожи, глаз, позу)
        2. Обращай внимание на признаки здоровья или возможных проблем
        3. Давай конкретные рекомендации по уходу
        4. Если видишь что-то тревожное - рекомендуй обратиться к ветеринару
        5. Предлагай профилактические меры
        6. Если человек обращается с вопросом о возможной болезни описывая состояние или прикладывая фото, давай диагноз, если знаешь
        Отвечай подробно и на русском языке."""
        
        user_prompt = f"""Внимательно изучи эту фотографию питомца и проанализируй:
        - Что ты видишь на изображении?
        - Какое общее состояние животного?
        - Есть ли что-то, на что стоит обратить внимание?
        - Какие рекомендации можешь дать?
        
        {text_prompt if text_prompt else ""}"""
        ...
        return "Не смог проанализировать фото. Попробуй еще раз."
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state != "DONE":
        await update.message.reply_text("Сначала завершите настройку с помощью /start")
        return
    ...
    try:
        ...
        await update.message.reply_text("Не смог обработать фотографию. Попробуй еще раз.")
...
def main():
    ...
    app.add_handler(MessageHandler(filters.PHOTO, handle_photo))  # Добавляем обработчик фото
    app.run_polling()
